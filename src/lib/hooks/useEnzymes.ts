...
export function useEnzymeSearch(step: Pick<PathwayStep, 'reactionType' | 'fromSubstrate' | 'toSubstrate'> | null) {
  return useQuery({
    queryKey: ['enzymes', 'search', step?.reactionType, step?.fromSubstrate, step?.toSubstrate],
